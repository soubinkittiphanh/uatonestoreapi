...
const fetchWaletTxnCRnDR = async (req, res) => {
    ...
    const sqlCom=`SELECT h.txn_his_id,c.txn_sign,c.txn_code_name,h.txn_his_amount,h.txn_his_date FROM transaction_history h LEFT JOIN transaction t ON t.txn_id=h.txn_id LEFT JOIN transaction_code c ON c.txn_code_id=t.txn_code WHERE h.user_id='${userId}' ORDER BY h.txn_his_date`
